...
% This wrapper keeps the original WSINDy interface but adds a final
% coefficient clip. The main stabilization should still come from a
% smaller library and ridge gamma in the caller.
max_abs_coef = 50;
...
%           Cov = Cov + alph_RT*speye(length(grid_i));
    ...
    %%% regress
    if isscalar(lambda)
        [w_sparse(:,nn),~] = sparsifyDynamics(G,b,lambda,1,gamma,M_diag);
        loss_wsindy = [];
    else
        if ~isempty(M_diag)
            M_scale_b = [1;M_diag];
...
%       alpha_loss = fzero(@(a) (1-a)./a-0.02*size(G,2),[0.01 1])
        [w_sparse(:,nn),loss_wsindy{nn},~] = wsindy_pde_RGLS_seq(lambda,gamma,[b G],1,M_scale_b,alpha_loss);
    end

    over = abs(w_sparse(:,nn)) > max_abs_coef;
    if any(over)
        fprintf('WSINDy cap: clipping %d coefficients for state %d above %.1f.\n', ...
            nnz(over), nn, max_abs_coef);
        w_sparse(over,nn) = max(min(w_sparse(over,nn), max_abs_coef), -max_abs_coef);
    end
...
if run_sindy
    ...
    for nn=1:n
        if isscalar(lambda)
            [w_sparse_sindy(:,nn),~] = sparsifyDynamics(Theta_sindy,dxobs_0(:,nn),lambda,1,gamma,M_diag(2:end));
            loss_sindy = [];
        else
            if ~isempty(M_diag)
                M_scale_b = [1;M_diag];
            ...
            [w_sparse_sindy(:,nn),loss_sindy{nn},~] = wsindy_pde_RGLS_seq(lambda,gamma,[dxobs_0(:,nn) Theta_sindy],1,M_scale_b,alpha_loss);
        end

        over = abs(w_sparse_sindy(:,nn)) > max_abs_coef;
        if any(over)
            w_sparse_sindy(over,nn) = max(min(w_sparse_sindy(over,nn), max_abs_coef), -max_abs_coef);
        end
    end
    ET_sindy = toc;
else
